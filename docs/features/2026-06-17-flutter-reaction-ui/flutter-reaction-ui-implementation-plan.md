...
**Goal:** ulssu 글 상세 화면에 좋아요/싫어요 버튼을 붙여 reaction API를 호출하고, 새로 늘어난 AI 댓글만 기존 순차 등장 연출로 이어 붙인다. 수치는 비노출, 잠긴 글도 버튼 유지, 게시판 톤.
...
**Files:**
- Create: `ulssu/lib/services/api.dart`
- Test: `ulssu/test/services/api_test.dart`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `ulssu/test/services/api_test.dart`):
```dart
import 'dart:convert';
...
Run: `cd ulssu && flutter test test/services/api_test.dart`
Expected: FAIL — `Error: Couldn't resolve the package 'ulssu/services/api.dart'` (모듈 미존재, 컴파일 실패)

- [ ] **Step 3: ApiService 구현**

**수정 후** (new file: `ulssu/lib/services/api.dart`):
```dart
import 'dart:convert';
...
```bash
git add ulssu/lib/services/api.dart ulssu/test/services/api_test.dart
...
**Files:**
- Modify: `ulssu/lib/screens/home_screen.dart:1-4` (imports), `:13-16` (필드), `:31-45` (getPosts), `:58-82` (createPost)
...
**원본** (`ulssu/lib/screens/home_screen.dart:1-4`):
```dart
...
**원본** (`ulssu/lib/screens/home_screen.dart:13-16`):
```dart
...
**원본** (`ulssu/lib/screens/home_screen.dart:31-42`):
```dart
    try {
...
**원본** (`ulssu/lib/screens/home_screen.dart:58-79`):
```dart
    try {
      final url = Uri.parse('http://172.28.0.1:8000/api/posts');
...
**Files:**
- Modify: `ulssu/lib/screens/detail_screen.dart` (전체 재작성 — 생성자/상태/initState/메서드/build)
- Modify: `ulssu/lib/screens/home_screen.dart:164-168` (DetailScreen push에 postId 전달)
- Test: `ulssu/test/screens/detail_reaction_test.dart`

**Model**: sonnet

- [ ] **Step 1: 실패하는 위젯 테스트 작성**

**수정 후** (new file: `ulssu/test/screens/detail_reaction_test.dart`):
```dart
import 'dart:convert';
...
Run: `cd ulssu && flutter test test/screens/detail_reaction_test.dart`
Expected: FAIL — DetailScreen에 `postId`/`api` 파라미터, `Key('like-button')` 부재로 컴파일/매칭 실패

- [ ] **Step 3: detail_screen 전체 재작성**

**원본** (`ulssu/lib/screens/detail_screen.dart:1-143`):
```dart
import 'package:flutter/material.dart';
...
**원본** (`ulssu/lib/screens/home_screen.dart:164-168`):
```dart
...
```bash
git add ulssu/lib/screens/detail_screen.dart ulssu/lib/screens/home_screen.dart ulssu/test/screens/detail_reaction_test.dart
...
- `ulssu/lib/screens/detail_screen.dart:_react` — **side-effect**: `reactToPost` 1회가 백엔드 동기 AI 생성을 유발해 응답이 수초 걸릴 수 있음. (mitigation: `_isReacting` 플래그로 처리 중 버튼 비활성 + 인디케이터, 응답 후 해제. 속도 비요구(NFR)라 지연 허용.)
- `ulssu/lib/screens/home_screen.dart:26-79` — **breaking**: getPosts/createPost를 ApiService로 이관 → 응답 파싱/에러 처리 동작이 바뀔 위험. (mitigation: 계약 동일 유지(반환 타입·키), Task 1 유닛테스트로 서비스 검증 + `flutter analyze` + 수동 확인.)
- `ulssu/lib/screens/detail_screen.dart:_react` / `_revealNewComments` — **race**: 화면 dispose 후 비동기 응답/딜레이 콜백 도착 시 `setState` 예외. (mitigation: 각 await 뒤 `if (!mounted) return` / `if (mounted)` 가드.)
...
- **영향범위**: ulssu/ — api.dart(신설), home_screen·detail_screen(수정), test 2파일. 실행순서 결함(중간 커밋 컴파일) 자체점검에서 수정.
...
- **무엇이**: `ulssu/lib/services/api.dart`(신설), `ulssu/lib/screens/home_screen.dart`, `ulssu/lib/screens/detail_screen.dart`, `ulssu/test/services/api_test.dart`(신설), `ulssu/test/screens/detail_reaction_test.dart`(신설), `ulssu/test/widget_test.dart`(삭제)
