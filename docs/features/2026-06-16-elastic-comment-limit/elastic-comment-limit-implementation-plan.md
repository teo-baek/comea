...
- Create: `comea_backend/elastic_limit.py`
- Test: `comea_backend/tests/test_elastic_limit.py`
...
**수정 후** (new file: `comea_backend/tests/test_elastic_limit.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_elastic_limit.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'elastic_limit'`

- [ ] **Step 4: 최소 구현 작성**

**수정 후** (new file: `comea_backend/elastic_limit.py`):
```python
...
```bash
git add pyproject.toml uv.lock comea_backend/elastic_limit.py comea_backend/tests/test_elastic_limit.py
...
**Files:**
- Create: `comea_backend/population.py`
- Test: `comea_backend/tests/test_population.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_population.py`):
```python
import pytest
...
Run: `cd comea_backend && uv run pytest tests/test_population.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'population'`

- [ ] **Step 3: 최소 구현 작성**

**수정 후** (new file: `comea_backend/population.py`):
```python
...
```bash
git add comea_backend/population.py comea_backend/tests/test_population.py
...
**Files:**
- Create: `comea_backend/personas.py`
- Test: `comea_backend/tests/test_personas.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_personas.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_personas.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'personas'`

- [ ] **Step 3: 최소 구현 작성**

**수정 후** (new file: `comea_backend/personas.py`):
```python
...
```bash
git add comea_backend/personas.py comea_backend/tests/test_personas.py
...
**Files:**
- Create: `comea_backend/comment_style.py`
- Test: `comea_backend/tests/test_comment_style.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_comment_style.py`):
```python
import random
...
Run: `cd comea_backend && uv run pytest tests/test_comment_style.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'comment_style'`

- [ ] **Step 3: 최소 구현 작성**

**수정 후** (new file: `comea_backend/comment_style.py`):
```python
"""댓글 분량(길이) 변주 스타일.
...
```bash
git add comea_backend/comment_style.py comea_backend/tests/test_comment_style.py
...
**Files:**
- Modify: `comea_backend/database.py:2`, `comea_backend/database.py:12`, `comea_backend/database.py:17-22`, `comea_backend/database.py:24-30`
- Test: `comea_backend/tests/test_schema.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_schema.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_schema.py -v`
...
**원본** (`comea_backend/database.py:12`):
```python
...
**원본** (`comea_backend/database.py:17-22`):
```python
...
**원본** (`comea_backend/database.py:24-30`):
```python
...
```bash
git add comea_backend/database.py comea_backend/tests/test_schema.py
...
**Files:**
- Modify: `comea_backend/main.py:25-26`
- Create: `comea_backend/tests/conftest.py`
- Test: `comea_backend/tests/test_smoke.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 스모크 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_smoke.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_smoke.py -v`
Expected: FAIL — `fixture 'client' not found`

- [ ] **Step 3: 하드코딩된 OpenAI 키 제거 (env 사용)**

**원본** (`comea_backend/main.py:25-26`):
```python
os.environ["OPENAI_API_KEY"] = "sk-proj-<REDACTED — 실제 키였음, 폐기 대상>"
client = OpenAI()
```
...
**수정 후** (new file: `comea_backend/tests/conftest.py`):
```python
import os
...
```bash
git add comea_backend/main.py comea_backend/tests/conftest.py comea_backend/tests/test_smoke.py
...
**Files:**
- Modify: `comea_backend/main.py:9-10` (imports), `comea_backend/main.py:57-68` (generate_ai_comment), `comea_backend/main.py:77-113` (create_post)
- Test: `comea_backend/tests/test_create_post.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 통합 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_create_post.py`):
```python
import main
...
Run: `cd comea_backend && uv run pytest tests/test_create_post.py -v`
...
**원본** (`comea_backend/main.py:57-68`):
```python
...
**원본** (`comea_backend/main.py:77-113`):
```python
@app.post("/api/posts")
...
```bash
git add comea_backend/main.py comea_backend/tests/test_create_post.py
...
**Files:**
- Modify: `comea_backend/main.py:3` (HTTPException), `comea_backend/main.py:28-29` (ReactionRequest), generate_ai_comment 아래(헬퍼), 파일 끝(라우트)
- Test: `comea_backend/tests/test_reaction_api.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 통합 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_reaction_api.py`):
```python
import main
...
Run: `cd comea_backend && uv run pytest tests/test_reaction_api.py -v`
Expected: FAIL — `405`/`404` (라우트 미존재)

- [ ] **Step 3: HTTPException import 보강**

**원본** (`comea_backend/main.py:3`):
```python
...
**원본** (`comea_backend/main.py:28-29`):
```python
...
**원본** (`comea_backend/main.py` — Task 7 적용 후의 generate_ai_comment):
```python
def generate_ai_comment(persona_prompt: str, user_post: str, previous_comments: str, length_hint: str) -> str:
    system_instruction = (
        "너는 'AI 광장'이라는 커뮤니티의 시민이야. 아래 페르소나에 맞춰 댓글을 달아줘.\n"
...
**수정 후** (append to `comea_backend/main.py`):
```python
...
```bash
git add comea_backend/main.py comea_backend/tests/test_reaction_api.py
...
**Files:**
- Test: `comea_backend/tests/test_lock_and_scale.py`
...
**수정 후** (new file: `comea_backend/tests/test_lock_and_scale.py`):
```python
import main
import population
...
Run: `cd comea_backend && uv run pytest tests/test_lock_and_scale.py -v`
Expected: PASS (3 passed). FAIL이면 Task 8의 `react_to_post` 종료/생성 분기 또는 `population` 사용을 점검.

- [ ] **Step 3: 전체 테스트 스위트 실행**

Run: `cd comea_backend && uv run pytest -v`
Expected: PASS (전체 green — 단위 + 통합)

- [ ] **Step 4: 커밋**

```bash
git add comea_backend/tests/test_lock_and_scale.py
...
- `comea_backend/main.py:react_to_post` / `_generate_more_comments` — **side-effect**: 반응 처리 중 동기 OpenAI 호출로 댓글을 생성. 속도는 비요구(FR-10)라 지연 자체는 허용이나, 유저수가 큰 글(Cap이 수백+)에서 한 번에 다량 생성 시 API 비용이 급증. (mitigation: `compute_final_limit`이 증감률을 반응 1건당 +0.1로 점증시켜 요청당 델타를 작게 유지. 비용 상한이 필요하면 요청당 생성 델타 cap 추가 — 후속 TODO.)
- `comea_backend/database.py:17-30` — **breaking**: `posts.is_locked` 추가 + `reactions` 테이블 신설. 기존 운영 PostgreSQL은 `Base.metadata.create_all`이 컬럼 ALTER/테이블 신설을 보장하지 않아 누락 가능. 응답에 `is_locked` 가산. (mitigation: dev DB는 재생성으로 충분. 운영은 수동 `ALTER TABLE posts ADD COLUMN is_locked ...` + `CREATE TABLE reactions ...` 또는 Alembic — §3 롤백 참조.)
- `comea_backend/main.py:react_to_post` (생성 경로) — **race**: 반응 적재는 스택 INSERT로 경합이 없으나(FR-9), 두 반응 요청이 거의 동시에 "부족분 생성"을 계산하면 Cap을 일시 초과해 생성할 수 있음. (mitigation: 생성 후 `should_lock` 재집계로 Cap 도달 시 잠금. 엄밀한 상한 보장이 필요하면 글 행 `SELECT ... FOR UPDATE`로 생성 구간 직렬화 — 후속.)
...
- **영향범위**: 동일 폴더 `elastic-comment-limit-requirements.md`(FR-1~13 + AC-1~8), `elastic-comment-limit-tech-design.md`(D1~7) 동기 개정. 구현 대상 코드: `comea_backend/`(elastic_limit·population·personas·comment_style 신규, database·main 수정)
- **연관 항목**: 없음 (최초 엔트리)
...
- **무엇이**: `comea_backend/elastic_limit.py`, `population.py`, `personas.py`, `comment_style.py`, `database.py`, `main.py`, `conftest.py`, `tests/*`(8 파일), `pyproject.toml`, `uv.lock`
...
- **계획 대비 보정 2건** (실행 중 발견):
  - `comea_backend/conftest.py`(루트, 빈 파일) 추가 — 평면 import 모듈을 tests/에서 import 가능하게 sys.path 보장 (Task 1)
...
### [2026-06-17 01:30] [코드-수정] (trivial)
- **id**: CH-20260617-002
- **이유**: 사용자가 생성한 `.env`(OPENAI_API_KEY)가 실제로 로드되도록 python-dotenv 연동 + PoC 잔재(battle_arena*.py, 루트 main.py) 정리. `.env`는 .gitignore 포함, PoC 파일은 untracked라 git 히스토리 무관.
- **무엇이**: `comea_backend/main.py:8` (load_dotenv 추가, database import 전), `pyproject.toml`/`uv.lock`(python-dotenv 의존성), 삭제: `battle_arena.py`·`battle_arena_v2.py`·루트 `main.py`
- **비고**: `load_dotenv(override=False)`라 테스트의 sqlite/더미 키 격리 유지. 전체 35 passed 재확인. (스키마/로직 변경 없음 → trivial)

### [2026-06-17 01:45] [구현계획서-수정]
- **id**: CH-20260617-003
- **이유**: 보안 — Task 6 `원본` 블록(line 623)에 노출 OpenAI 키 평문이 그대로 들어가 커밋됐던 것을 마스킹(`sk-proj-<REDACTED>`). 계획서 작성 시 키를 가리지 않은 실수 정정.
- **무엇이**: `elastic-comment-limit-implementation-plan.md` Task 6 Step 3 원본 코드 블록
- **영향범위**: 문서 텍스트만. 워킹트리·향후 커밋은 키 미포함. **단, 이전 커밋(plan 최초 커밋들)의 git 히스토리에는 키가 잔존** → 완전 제거는 히스토리 재작성 필요(별도 논의, change-history의 commit SHA 참조가 깨지는 트레이드오프 있음).
- **비고**: 사용자가 새 키를 .env로 재발급함. 구 키 유효 시 revoke 권장.
