...
**Files:**
- Modify: `comea_backend/database.py:2` (import), `comea_backend/database.py` (CommentReactionModel)
- Modify: `comea_backend/main.py:15` (import), `comea_backend/main.py` 끝(엔드포인트)
- Create: `comea_backend/migrations/004_add_comment_reactions.sql`
- Test: `comea_backend/tests/test_comment_reaction.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 통합 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_comment_reaction.py`):
```python
import database
import main
...
Run: `cd comea_backend && uv run pytest tests/test_comment_reaction.py -v`
...
**수정 후** (new file: `comea_backend/migrations/004_add_comment_reactions.sql`):
```sql
...
```bash
git add comea_backend/database.py comea_backend/main.py comea_backend/migrations/004_add_comment_reactions.sql comea_backend/tests/test_comment_reaction.py
...
**Files:**
- Modify: `comea/lib/services/api.dart` (reactToComment)
- Modify: `comea/lib/screens/detail_screen.dart` (_commentReactions 상태 + _reactToComment + 타일 버튼)
- Test: `comea/test/services/api_comment_reaction_test.dart`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `comea/test/services/api_comment_reaction_test.dart`):
```dart
import 'dart:convert';
...
Run: `cd comea && flutter test test/services/api_comment_reaction_test.dart`
Expected: FAIL — `reactToComment` 미존재로 컴파일 실패

- [ ] **Step 3: ApiService에 reactToComment 추가**

**원본** (`comea/lib/services/api.dart` — reactToPost 메서드 끝):
```dart
...
**원본** (`comea/lib/screens/detail_screen.dart:30`):
```dart
...
**원본** (`comea/lib/screens/detail_screen.dart` — 댓글 본문 Container + Column 닫힘):
```dart
                                  Container(
...
```bash
git add comea/lib/services/api.dart comea/lib/screens/detail_screen.dart comea/test/services/api_comment_reaction_test.dart
...
- `comea_backend/database.py:CommentReactionModel` — **breaking**: `comment_reactions` 테이블 신설. 운영 DB `create_all` 미반영 가능. (mitigation: `migrations/004_*.sql` 멱등. dev는 재생성.)
- `comea_backend/main.py:react_to_comment` / `comea/lib/screens/detail_screen.dart:_reactToComment` — **side-effect**: 댓글마다 버튼 + API 호출. (mitigation: 수집 전용 — 한계선/댓글 생성 무영향(별도 테이블·경로). Flutter 실패 시 로컬 상태 원복 + 스낵바.)
...
- **영향범위**: comea_backend(comment_reactions·엔드포인트·migration 004)·comea(api·detail_screen 타일 버튼).
...
- **무엇이**: `comea_backend/database.py`(CommentReactionModel+UniqueConstraint), `main.py`(엔드포인트+import), `migrations/004_*.sql`(신설), `tests/test_comment_reaction.py`(신설), `comea/lib/services/api.dart`(reactToComment), `screens/detail_screen.dart`(타일 버튼+상태+핸들러), `test/services/api_comment_reaction_test.dart`(신설)
