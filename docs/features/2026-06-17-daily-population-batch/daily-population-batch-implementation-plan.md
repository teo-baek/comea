...
- Create: `comea_backend/population_batch.py`
- Test: `comea_backend/tests/test_population_batch.py`
...
**수정 후** (new file: `comea_backend/tests/test_population_batch.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_population_batch.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'population_batch'`

- [ ] **Step 4: 구현 작성**

**수정 후** (new file: `comea_backend/population_batch.py`):
```python
...
```bash
git add pyproject.toml uv.lock comea_backend/population_batch.py comea_backend/tests/test_population_batch.py
...
**Files:**
- Modify: `comea_backend/main.py:16` (import), `comea_backend/main.py:35-36` (이벤트 핸들러 추가)
- Modify: `comea_backend/tests/conftest.py:5` (DISABLE_SCHEDULER env)

**Model**: sonnet

- [ ] **Step 1: conftest에 DISABLE_SCHEDULER 가드 추가**

**원본** (`comea_backend/tests/conftest.py:1-6`):
```python
import os
...
**원본** (`comea_backend/main.py:35-36`):
```python
...
```bash
git add comea_backend/main.py comea_backend/tests/conftest.py
...
- `comea_backend/population_batch.py:start_scheduler` — **side-effect**: 기동 시 APScheduler 스레드 + 즉시 DB 집계. (mitigation: `DISABLE_SCHEDULER` env 가드로 테스트 미기동(D5), `run_population_update` try/except 예외격리(AC-5), 종료 이벤트로 스레드 정리.)
- `comea_backend/population.py:current_population` (소비처) — **race**: 인메모리 전역이라 멀티워커 시 워커별 상이. (mitigation: 단일 프로세스 가정(NFR/범위 밖). 운영 멀티워커는 후속 Redis 공유로 별도 처리.)
...
- **영향범위**: comea_backend(population_batch 신설·main 이벤트·conftest 가드)·apscheduler 의존성.
...
- **무엇이**: `comea_backend/population_batch.py`(신설), `main.py`(기동/종료 이벤트), `tests/conftest.py`(DISABLE_SCHEDULER), `tests/test_population_batch.py`(신설), `pyproject.toml`/`uv.lock`(apscheduler)
