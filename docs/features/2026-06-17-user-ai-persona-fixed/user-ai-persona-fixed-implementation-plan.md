...
**Files:**
- Modify: `ulssu_backend/database.py:2` (JSON import), `ulssu_backend/database.py:15` (UserModel 아래 AiPersonaModel)
- Modify: `ulssu_backend/personas.py` (random_persona 추가)
- Create: `ulssu_backend/migrations/003_add_ai_personas.sql`
- Test: `ulssu_backend/tests/test_persona.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `ulssu_backend/tests/test_persona.py`):
```python
import random
...
Run: `cd ulssu_backend && uv run pytest tests/test_persona.py -v`
...
**원본** (`ulssu_backend/database.py:13-16`):
```python
...
**원본** (`ulssu_backend/personas.py:1`):
```python
...
**수정 후** (append to `ulssu_backend/personas.py`):
```python
...
**수정 후** (new file: `ulssu_backend/migrations/003_add_ai_personas.sql`):
```sql
...
```bash
git add ulssu_backend/database.py ulssu_backend/personas.py ulssu_backend/migrations/003_add_ai_personas.sql ulssu_backend/tests/test_persona.py
...
**Files:**
- Modify: `ulssu_backend/main.py:15-16` (import), `ulssu_backend/main.py:139-148` (signup)
- Test: `ulssu_backend/tests/test_persona_signup.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 통합 테스트 작성**

**수정 후** (new file: `ulssu_backend/tests/test_persona_signup.py`):
```python
import database
import main
from database import AiPersonaModel, UserModel
...
**원본** (`ulssu_backend/main.py:140-148`):
```python
...
```bash
git add ulssu_backend/main.py ulssu_backend/tests/test_persona_signup.py
...
- `ulssu_backend/database.py:AiPersonaModel` — **breaking**: `ai_personas` 테이블 신설. 운영 DB `create_all` 미반영 가능. (mitigation: `migrations/003_*.sql` 멱등 SQL. dev는 재생성.)
- `ulssu_backend/main.py:signup` — **side-effect**: 가입이 추가 write(페르소나) 수행. (mitigation: 유저 commit 선행 + 페르소나 try/except best-effort로 가입 흐름 보호. 실패 시 rollback.)
...
- **영향범위**: ulssu_backend(database AiPersonaModel·personas·main signup·migration 003).
- **연관 항목**: CH-20260617-001

### [2026-06-17 13:35] [코드-수정] (batch: tasks 1..2)
- **id**: CH-20260617-003
- **이유**: 유저별 AI 페르소나(중간=고정·내부) 구현(2 task). 가입 시 풀 랜덤 페르소나 1개 내부 생성·저장.
- **무엇이**: `ulssu_backend/database.py`(AiPersonaModel+JSON), `personas.py`(random_persona), `main.py`(signup 훅), `migrations/003_add_ai_personas.sql`(신설), `tests/test_persona.py`·`test_persona_signup.py`(신설)
