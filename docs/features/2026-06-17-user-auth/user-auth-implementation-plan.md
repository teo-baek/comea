...
- Create: `comea_backend/auth.py`
- Test: `comea_backend/tests/test_auth.py`
...
**수정 후** (new file: `comea_backend/tests/test_auth.py`):
```python
import jwt
import pytest
...
Run: `cd comea_backend && uv run pytest tests/test_auth.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'auth'`

- [ ] **Step 4: 순수 로직 구현**

**수정 후** (new file: `comea_backend/auth.py`):
```python
...
```bash
git add pyproject.toml uv.lock comea_backend/auth.py comea_backend/tests/test_auth.py
...
**Files:**
- Modify: `comea_backend/database.py:19-26` (PostModel), `comea_backend/database.py:37-44` (ReactionModel), `comea_backend/database.py:47` (get_db 위 UserModel 추가)
- Create: `comea_backend/migrations/002_add_users_and_authorship.sql`
- Test: `comea_backend/tests/test_user_schema.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_user_schema.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_user_schema.py -v`
Expected: FAIL — `ImportError: cannot import name 'UserModel'`

- [ ] **Step 3: PostModel에 author_user_id 추가**

**원본** (`comea_backend/database.py:19-26`):
```python
...
**원본** (`comea_backend/database.py:37-44`):
```python
...
**원본** (`comea_backend/database.py:46-47`):
```python
...
**수정 후** (new file: `comea_backend/migrations/002_add_users_and_authorship.sql`):
```sql
...
```bash
git add comea_backend/database.py comea_backend/migrations/002_add_users_and_authorship.sql comea_backend/tests/test_user_schema.py
...
**Files:**
- Modify: `comea_backend/auth.py` (get_current_user 추가)
- Modify: `comea_backend/main.py:37-42` (요청 모델), `comea_backend/main.py:115` 앞(인증 라우트)
- Modify: `comea_backend/tests/conftest.py` (JWT_SECRET env + auth_client 픽스처)
- Test: `comea_backend/tests/test_auth_api.py`

**Model**: sonnet

- [ ] **Step 1: 실패하는 통합 테스트 작성**

**수정 후** (new file: `comea_backend/tests/test_auth_api.py`):
```python
...
Run: `cd comea_backend && uv run pytest tests/test_auth_api.py -v`
Expected: FAIL — `404` (auth 라우트 미존재)

- [ ] **Step 3: auth.py에 get_current_user 추가**

**원본** (`comea_backend/auth.py:1`):
```python
"""인증 순수 로직: 비밀번호 해시(bcrypt) + JWT(PyJWT). get_current_user 는 Task 3에서 추가."""
import os
import datetime as dt
...
**원본** (`comea_backend/auth.py:decode_token 끝`):
```python
def decode_token(token: str) -> int:
    payload = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
    return int(payload["sub"])
...
**원본** (`comea_backend/main.py:37-42`):
```python
...
**원본** (`comea_backend/main.py:115-116`):
```python
...
**원본** (`comea_backend/tests/conftest.py:1-5`):
```python
import os
...
**원본** (`comea_backend/tests/conftest.py` 끝 — `client` 픽스처의 마지막 두 줄):
```python
...
```bash
git add comea_backend/auth.py comea_backend/main.py comea_backend/tests/conftest.py comea_backend/tests/test_auth_api.py
...
**Files:**
- Modify: `comea_backend/main.py:122-148` (create_post), `comea_backend/main.py:152-181` (react_to_post)
- Modify: `comea_backend/tests/test_create_post.py`, `test_reaction_api.py`, `test_lock_and_scale.py` (auth_client로 전환)

**Model**: sonnet

- [ ] **Step 1: create_post 보호 + author 연결**

**원본** (`comea_backend/main.py:122-135`):
```python
@app.post("/api/posts")
...
**원본** (`comea_backend/main.py:152-161`):
```python
...
**원본** (`comea_backend/tests/test_auth_api.py` 끝):
```python
    bad = client.post("/api/auth/login", json={"email": "u2@x.com", "password": "WRONG"})
    assert bad.status_code == 401
```
...
**원본** (`comea_backend/tests/test_create_post.py` — 함수 시그니처들):
```python
...
```bash
git add comea_backend/main.py comea_backend/tests/test_create_post.py comea_backend/tests/test_reaction_api.py comea_backend/tests/test_lock_and_scale.py comea_backend/tests/test_auth_api.py
...
**Files:**
- Modify: `comea/pubspec.yaml` (shared_preferences), `comea/lib/services/api.dart`
- Test: `comea/test/services/api_auth_test.dart`

**Model**: sonnet

- [ ] **Step 1: shared_preferences 추가**

Run: `cd comea && flutter pub add shared_preferences`
Expected: `pubspec.yaml` dependencies에 `shared_preferences` 추가.

- [ ] **Step 2: 실패하는 테스트 작성**

**수정 후** (new file: `comea/test/services/api_auth_test.dart`):
```dart
import 'dart:convert';
...
Run: `cd comea && flutter test test/services/api_auth_test.dart`
Expected: FAIL — `login`/`token` 미존재로 컴파일 실패

- [ ] **Step 4: ApiService에 인증 추가**

**원본** (`comea/lib/services/api.dart:1-31`):
```dart
import 'dart:convert';
...
**원본** (`comea/lib/services/api.dart:reactToPost 내부 headers`):
```dart
...
```bash
git add comea/pubspec.yaml comea/pubspec.lock comea/lib/services/api.dart comea/test/services/api_auth_test.dart
...
**Files:**
- Create: `comea/lib/screens/login_screen.dart`
- Modify: `comea/lib/main.dart` (토큰 유무 분기 + 토큰 영속화)
- Test: `comea/test/screens/login_test.dart`

**Model**: sonnet

- [ ] **Step 1: 실패하는 위젯 테스트 작성**

**수정 후** (new file: `comea/test/screens/login_test.dart`):
```dart
import 'dart:convert';
...
import 'package:http/http.dart' as http;
import 'package:http/testing.dart';
import 'package:comea/screens/login_screen.dart';
import 'package:comea/services/api.dart';
...
Run: `cd comea && flutter test test/screens/login_test.dart`
Expected: FAIL — `LoginScreen` 미존재

- [ ] **Step 3: login_screen.dart 생성**

**수정 후** (new file: `comea/lib/screens/login_screen.dart`):
```dart
import 'package:flutter/material.dart';
...
**원본** (`comea/lib/screens/home_screen.dart:6-16`):
```dart
...
**원본** (`comea/lib/screens/home_screen.dart` — 앱바 actions의 새로고침 IconButton):
```dart
        actions: [
          // 새로고침 버튼 추가
          IconButton(
...
```bash
git add comea/lib/main.dart comea/lib/screens/login_screen.dart comea/lib/screens/home_screen.dart comea/test/screens/login_test.dart
...
- `comea_backend/main.py:create_post,react_to_post` — **breaking**: 인증 강제로 기존 무인증 호출(테스트·Flutter)이 401. (mitigation: 같은 슬라이스에서 conftest `auth_client` + 기존 테스트 전환(Task 4) + Flutter 헤더 주입(Task 5). e2e로 확인.)
- `comea_backend/database.py:users/FK` — **breaking**: users 테이블 + FK 컬럼 추가. 운영 DB `create_all` 미반영. (mitigation: `migrations/002_*.sql` 멱등 SQL + 런북. dev는 재생성.)
- `comea_backend/auth.py` — **side-effect(security)**: 비번 해시·JWT 시크릿 취급. (mitigation: bcrypt 해시·평문 미저장, `JWT_SECRET` env(기본값은 dev용 — 운영은 반드시 교체), 토큰 만료 7일. password_hash 응답 비노출.)
...
- **영향범위**: comea_backend(auth/database/main/conftest/기존 테스트)·migrations/002·comea(api/screens/main)·신규 의존성 3. 북극성 §4 정합.
...
- **무엇이**: `comea_backend/auth.py`(신설)·`database.py`·`main.py`·`tests/conftest.py`·기존 테스트 3파일(전환)·`tests/test_auth*.py`·`tests/test_user_schema.py`(신설)·`migrations/002_*.sql`(신설), `comea/lib/services/api.dart`·`screens/login_screen.dart`(신설)·`screens/home_screen.dart`·`main.dart`·`test/*`(신설), `pyproject.toml`/`pubspec.yaml`
...
- **task별 세부 (6건)**:
  ...
  - UA5: `comea/lib/services/api.dart` — Flutter 인증 메서드+헤더 (none) — `49e324a`
