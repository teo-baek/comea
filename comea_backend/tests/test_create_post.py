...
def test_create_post_chitchat_still_gets_ten_comments(auth_client, monkeypatch):
    ...
    resp = auth_client.post("/api/posts", json={"content": "돈까스 땡긴다"})
...
def test_create_post_normal_gets_fifteen(auth_client, monkeypatch):
    monkeypatch.setattr(main, "evaluate_post_quality", lambda p: 70)  # 일반 base 15
    body = auth_client.post("/api/posts", json={"content": "이직 고민"}).json()
    assert len(body["comments"]) == 15


def test_create_post_hot_gets_twenty(auth_client, monkeypatch):
    monkeypatch.setattr(main, "evaluate_post_quality", lambda p: 95)  # 명글 base 20
    body = auth_client.post("/api/posts", json={"content": "영끌 주식 마이너스 20%"}).json()
    assert len(body["comments"]) == 20


def test_create_post_response_hides_reaction_counts(auth_client, monkeypatch):
    monkeypatch.setattr(main, "evaluate_post_quality", lambda p: 70)
    body = auth_client.post("/api/posts", json={"content": "테스트"}).json()
    assert "like_count" not in body
    assert "dislike_count" not in body
    assert "reactions" not in body  # 반응 카운트/목록 비노출 (FR-3)


def test_create_post_comments_use_length_styles(auth_client, monkeypatch):
    ...
    auth_client.post("/api/posts", json={"content": "테스트"})
